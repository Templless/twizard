 ...
 */
$item = ! empty( twiz_interface()->data['advanced-item'] ) ? twiz_interface()->data['advanced-item'] : false;
$slug = ! empty( twiz_interface()->data['advanced-slug'] ) ? twiz_interface()->data['advanced-slug'] : false;
...
$full_path = twiz_tools()->secure_path( $xml_full );
$min_path  = twiz_tools()->secure_path( $xml_min );
...
<div class="advanced-item" data-full="<?php echo $full_path; ?>" data-lite="<?php echo $min_path; ?>">
	...
	<div class="advanced-item__content">
		...
		<div class="advanced-item__recommended-plugins"><?php
			esc_html_e( 'Recommended Plugins:', 'twizard-importer' );
		?></div>
		...
		<div class="advanced-item__install-type">
			<label class="advanced-item__type-checkbox">
				<input type="checkbox"><?php esc_html_e( 'Optimize Demo Content', 'twizard-importer' ); ?>
			</label>
			<?php esc_html_e( 'Please select this option to install light version of demo content. Recommended for slow severs and shared web hosts', 'twizard-importer' ); ?>
		</div>
		<div class="advanced-item__install">
			<button class="twiz-btn primary" data-action="start-install"><span class="text"><?php
				esc_html_e( 'Install Demo', 'twizard-importer' );
			?></span><span class="twiz-loader-wrapper-alt"><span class="twiz-loader-alt"></span></span></button>
			<a href="<?php echo $demo_url; ?>" target="_blank" class="twiz-btn"><?php
				esc_html_e( 'View Demo', 'twizard-importer' );
			?></a>
		</div>
	</div>
</div>
