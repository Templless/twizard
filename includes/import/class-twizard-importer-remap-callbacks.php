...
if ( ! class_exists( 'Twizard_Importer_Callbacks' ) ) {

	/**
	 * Define Twizard_Importer_Callbacks class
	 */
	class Twizard_Importer_Callbacks {
		...
		public function __construct() {
			...
			add_action( 'twizard_data_import_remap_posts', array( $this, 'process_options' ) );
			add_action( 'twizard_data_import_remap_posts', array( $this, 'postprocess_posts' ) );
			add_action( 'twizard_data_import_remap_posts', array( $this, 'process_thumbs' ) );
			add_action( 'twizard_data_import_remap_posts', array( $this, 'process_elementor_pages_posts' ) );
			add_action( 'twizard_data_import_remap_posts', array( $this, 'process_home_page' ) );

			// Manipulations with terms remap array
			add_action( 'twizard_data_import_remap_terms', array( $this, 'process_term_parents' ) );
			add_action( 'twizard_data_import_remap_terms', array( $this, 'process_nav_menu' ) );
			add_action( 'twizard_data_import_remap_terms', array( $this, 'process_nav_menu_widgets' ) );
			add_action( 'twizard_data_import_remap_terms', array( $this, 'process_home_page' ) );
		...
		public function elementor_pages() {
			...
			if ( null === $this->pages ) {
				$this->pages = get_posts( array(
					'post_type'      => array( 'page', 'twiz-theme-core', 'elementor_library' ),
					'posts_per_page' => -1,
				) );
			}
		...
		public function process_elementor_pages_posts( $data ) {
			...
			foreach ( $pages as $page ) {
				...
				$ids_keys = apply_filters( 'twizard_importer_elementor_post_ids_to_remap', array(
					'panel_template_id',
					'item_template_id',
				) );
		...
		public function process_elementor_pages_terms( $data ) {
			...
			$pages    = $this->elementor_pages();
			$ids_keys = apply_filters( 'twizard_importer_elementor_terms_ids_to_remap', array(
				'category_ids',
				'menu',
				'nav_menu',
			) );
		...
		public function process_home_page( $data ) {
			...
			$regex = apply_filters( 'twizard_data_import_home_regex_replace', array() );

			if ( empty( $regex ) ) {
				return false;
			}

			$pages = apply_filters( 'twizard_data_import_add_pages_to_replace', array() );
		...
		public function postprocess_posts( $mapping ) {
			...
			$todo      = twiz_cache()->get( 'posts', 'requires_remapping' );
			$user_slug = twiz_cache()->get( 'user_slug', 'mapping' );
			$url_remap = twiz_cache()->get_group( 'url_remap' );
			...
			foreach ( $todo as $post_id => $_ ) {
				...
				if ( in_array( get_post_type( $post_id ), array( 'page', 'post' ) ) ) {
					...
					$updated_links = str_replace( twiz_cache()->get( 'home' ), home_url(), $old_links );
...
/**
 * Returns instance of Twizard_Importer_Callbacks
 *
 * @return object
 */
function twiz_remap_callbacks() {
	return Twizard_Importer_Callbacks::get_instance();
}

twiz_remap_callbacks();
