...
if ( ! class_exists( 'Twizard_Importer_Interface' ) ) {

	/**
	 * Define Twizard_Importer_Interface class
	 */
	class Twizard_Importer_Interface {
		...
		function __construct() {
			...
			add_action( 'wp_ajax_twizard-import-chunk', array( $this, 'import_chunk' ) );
			add_action( 'wp_ajax_twizard-regenerate-thumbnails', array( $this, 'regenerate_chunk' ) );
			add_action( 'wp_ajax_twizard-import-get-file-path', array( $this, 'get_file_path' ) );
			add_action( 'wp_ajax_twizard-import-remove-content', array( $this, 'remove_content' ) );
			add_action( 'twizard_importer_before_messages', array( $this, 'check_server_params' ) );
		...
		public function maybe_skip_installation() {
			...
			if ( ! isset( $_GET['page'] ) || twiz()->slug !== $_GET['page'] ) {
				return false;
			}
			...
			if ( isset( $_GET['type'] ) && 'skip' === $_GET['type'] ) {
				...
				require_once twiz()->path( 'includes/import/class-twizard-importer-extensions.php' );

				/**
				 * Hook before redirect on demo content installation skip
				 */
				do_action( 'twizard_data_import_before_skip_redirect' );

				wp_redirect( twiz()->page_url( array( 'step' => 4 ) ) );
				die();
			}
		...
		public function remove_content() {
			...
			if ( empty( $_REQUEST['password'] ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Password is empty', 'twizard-importer' ),
				) );
			}
			...
			if ( wp_check_password( $password, $data->user_pass, $user_id ) ) {
				twiz_tools()->clear_content();
				wp_send_json_success( array(
					'message' => esc_html__( 'Content successfully removed', 'twizard-importer' ),
					'slider'  => twiz_slider()->render( false ),
				) );
			} else {
				wp_send_json_error( array(
					'message' => esc_html__( 'Entered password is invalid', 'twizard-importer' ),
				) );
			}
		...
		public function advanced_popup() {
			...
			twiz()->get_template( 'advanced-popup.php' );
		}
		...
		public function check_server_params() {
			...
			$messages = '';
			$format   = esc_html__( '%1$s: %2$s required, yours - %3$s', 'twizard-importer' );

			foreach ( twiz_tools()->server_params() as $param => $data ) {
				$val = ini_get( $param );
				$val = (int) $val;
			...
			$heading = '<div class="twiz-server-messages__heading">' . esc_html__( 'Some parameters from your server don\'t meet the requirements:' ) . '</div>';

			echo '<div class="twiz-server-messages">' . $heading . $messages . '</div>';
		}
		...
		public function chunk_size() {
			...
			$size = twiz()->get_setting( array( 'import', 'chunk_size' ) );
			$size = intval( $size );

			if ( ! $size ) {
				return twiz()->chunk_size;
			} else {
				return $size;
			}
		...
		public function menu_page() {
			...
			twiz()->register_tab(
				array(
					'id'   => $this->slug,
					'name' => esc_html__( 'Import', 'twizard-importer' ),
		...
		public function dispatch() {
			...
			ob_start();
			twiz_tools()->get_page_title( '<h2 class="page-title">', '</h2>', true );

			wp_enqueue_script( 'twizard-import' );
		...
		private function import_before() {
			twiz()->get_template( 'import-before.php' );
		}
		...
		private function import_after() {
			twiz()->get_template( 'import-after.php' );
		}
		...
		private function import_step() {
			...
			$count        = twiz_cache()->get( 'total_count' );
			...
			twiz_cache()->update( 'chunks_count', $chunks_count );

			twiz()->get_template( 'import.php' );
		...
		public function regenerate_thumbnails() {
			...
			$count = (array) $count;
			$step  = twiz()->get_setting( array( 'import', 'regenerate_chunk_size' ) );
			$total = 0;
			...
			wp_localize_script( 'twizard-import', 'TwizardRegenerateData', array(
				'totalImg'   => $total,
				'totalSteps' => ceil( $total / $step ),
				'step'       => $step,
			) );

			twiz()->get_template( 'regenerate.php' );
		...
		private function validate_request() {
			...
			if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'twizard-import' ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'You don\'t have permissions to do this', 'twizard-importer' ),
				) );
			}
		...
		public function regenerate_chunk() {
			...
			foreach ( $required as $field ) {
				...
				if ( ! isset( $_REQUEST[ $field ] ) ) {
					wp_send_json_error( array(
						'message' => sprintf(
							esc_html__( '%s is missing in request', 'twizard-importer' ), $field
						),
					) );
				}
			...
			if ( $is_last ) {
				$data['redirect'] = twiz()->page_url( array( 'tab' => $this->slug, 'step' => 4 ) );
			}
		...
		public function import_chunk() {
			...
			if ( empty( $_REQUEST['chunk'] ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Chunk number is missing in request', 'twizard-importer' ),
				) );
			}

			$chunk     = intval( $_REQUEST['chunk'] );
			$chunks    = twiz_cache()->get( 'chunks_count' );
			...
			switch ( $chunk ) {
				...
				case $chunks:
					 ...
					 */
					do_action( 'twizard_data_import_finish' );
				...
				default:
					 ...
					 * Hook on last import chunk
					 */
					do_action( 'twizard_data_import_chunk', $chunk );
		...
		public function get_importer() {
			...
			require_once twiz()->path( 'includes/import/class-twizard-wxr-importer.php' );

			$options = array();
			$file    = null;

			if ( isset( $_REQUEST['file'] ) ) {
				$file = twiz_tools()->esc_path( esc_attr( $_REQUEST['file'] ) );
			}

			if ( ! $file || ! file_exists( $file ) ) {
				$file = twiz()->get_setting( array( 'xml', 'path' ) );
			}
			...
			return $this->importer = new Twizard_WXR_Importer( $options, $file );
		}
		...
		public function remap_all() {
			...
			require_once twiz()->path( 'includes/import/class-twizard-importer-remap-callbacks.php' );
			 ...
			 */
			do_action( 'twizard_data_import_remap_posts', twiz_cache()->get( 'posts', 'mapping' ) );
			 ...
			 */
			do_action( 'twizard_data_import_remap_terms', twiz_cache()->get( 'term_id', 'mapping' ) );
			 ...
			 */
			do_action( 'twizard_data_import_remap_comments', twiz_cache()->get( 'comments', 'mapping' ) );
			 ...
			 */
			do_action( 'twizard_data_import_remap_posts_meta', twiz_cache()->get( 'posts_meta', 'requires_remapping' ) );
			 ...
			 */
			do_action( 'twizard_data_import_remap_terms_meta', twiz_cache()->get( 'terms_meta', 'requires_remapping' ) );
		...
		public function get_welcome_message() {
			...
			if ( 0 === $files ) {
				$message = __( 'Upload XML file with demo content', 'twizard-importer' );
			}

			if ( 1 === $files ) {
				$message = __( 'We found 1 XML file with demo content in your theme, install it?', 'twizard-importer' );
			}

			if ( 1 < $files ) {
				$message = sprintf(
					__( 'We found %s XML files in your theme. Please select one of them to install', 'twizard-importer' ),
					$files
				);
			}

			return '<div class="twiz-message">' . $message . '</div>';
		...
		public function get_xml_count() {
			...
			$files = twiz()->get_setting( array( 'xml', 'path' ) );
		...
		public function get_import_files_select( $before = '<div>', $after = '</div>' ) {
			...
			foreach ( $files as $name => $file ) {
				$result .= sprintf( $item_format, twiz_tools()->secure_path( $file ), $name, $selected );
				$selected = '';
			}
		...
		public function get_import_file_input( $before = '<div>', $after = '</div>' ) {
			...
			if ( ! twiz()->get_setting( array( 'xml', 'use_upload' ) ) ) {
				return;
			}
			...
			$result .= '<button class="twiz-btn primary" id="twizard-file-upload">';
			$result .= esc_html__( 'Upload File', 'twizard-importer' );
			$result .= '</button>';
		...
		public function is_advanced_import() {
			$advanced = twiz()->get_setting( array( 'advanced_import' ) );
		...
		public function advanced_import() {
			...
			foreach ( $advanced as $slug => $item ) {
				...
				twiz()->get_template( 'import-advanced.php' );
			}
		...
		public function remove_content_form() {
			...
			twiz()->get_template( 'remove-content-form.php' );
		...
		public function get_file_path() {
			...
			if ( ! isset( $_REQUEST['file'] ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'XML file not passed', 'twizard-importer' ),
				) );
			}
			...
			wp_send_json_success( array(
				'path' => twiz_tools()->secure_path( $path ),
			) );
...
/**
 * Returns instance of Twizard_Importer_Interface
 *
 * @return object
 */
function twiz_interface() {
	return Twizard_Importer_Interface::get_instance();
}
