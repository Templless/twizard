...
if ( ! class_exists( 'Twizard_Importer_Extensions' ) ) {

	/**
	 * Define Twizard_Importer_Extensions class
	 */
	class Twizard_Importer_Extensions {
		...
		public function __construct() {
			...
			add_filter( 'twizard_import_skip_post', array( $this, 'prevent_import_errors' ), 10, 2 );

			// Clear fonts cache after import
			add_action( 'twizard_data_import_finish', array( $this, 'clear_fonts_cache' ) );

			// Allow switch Monstroid2 skins
			add_action( 'twizard_data_import_before_skip_redirect', array( $this, 'switch_skin_on_skip_import' ) );
		}
		...
		public function switch_skin_on_skip_import() {
			...
			$skin_file = get_stylesheet_directory() . '/twizard-style-switcher-pressets/' . $mapped_skin . '.json';
		...
		public function clear_fonts_cache() {
			delete_transient( 'twizard_google_fonts_url' );
		}
...
/**
 * Returns instance of Twizard_Importer_Extensions
 *
 * @return object
 */
function twiz_extensions() {
	return Twizard_Importer_Extensions::get_instance();
}
