 ...
 * Main importer class
 */
class Twizard_WXR_Importer extends WP_Importer {
	/**
	...
	public function __construct( $options = array(), $file = null ) {
		...
		$this->logger = new Twizard_Importer_Logger();
	...
	public function prepare_import() {
		...
		twiz_cache()->update( 'total_count', $count );
	...
	public function update_summary() {
		...
		twiz_cache()->update( 'import_summary', $summary );
	...
	public function update_processed_summary( $type ) {
		...
		$processed = twiz_cache()->get( 'processed_summary' );
		...
		twiz_cache()->update( 'processed_summary', $processed );
		...
		switch ( $this->reader->name ) {
			...
			default:
				...
				$current = apply_filters(
					'twizard-importer/increment-import-node',
					$current,
					$this->reader->name
				);
		...
		while ( $this->reader->read() ) {
			...
			switch ( $this->reader->name ) {
				case 'wp:wxr_version':
					...
					twiz_cache()->update( 'version', $version );
				...
				case 'generator':
					twiz_cache()->update( 'generator', $this->reader->readString() );
				...
				case 'blog_description':
					...
					twiz_cache()->update( 'description', $description );
				...
				case 'wp:base_site_url':
					twiz_cache()->update( 'siteurl', $this->reader->readString() );

					$this->next();
					break;

				case 'wp:base_blog_url':
					twiz_cache()->update( 'home', $this->reader->readString() );
				...
				default:
					do_action( 'twizard-importer/import-node/' . $this->reader->name, $this );
					break;
			}
	...
	protected function parse_author_node( $node ) {
		...
		$saved = twiz_cache()->update( $data['user_login'], $result, 'users' );
	...
	protected function maybe_clone_child_options( $option, $value ) {
		...
		$to_copy = apply_filters( 'twizard_importer_copy_options_to_child', array(
			'theme_mods_%s',
			'%s_sidebars',
		) );
	...
	protected function process_tables( $data ) {
		...
		foreach ( $data as $table => $values ) {
			...
			if ( ! twiz_tools()->is_db_table_exists( $table ) ) {
				continue;
			}
	...
	protected function process_widgets( $data ) {
		...
		// Hook before import
		do_action( 'twizard_importer_before_widgets_processing' );

		$data = apply_filters( 'twizard_importer_widgets_data', $data );

		// Get all available widgets site supports
		$available_widgets = twiz_tools()->available_widgets();
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( $sidebar_data = $this->sidebar_exists( $sidebar_id ) ) {
				...
				$sidebar_message      = esc_html__(
					'Sidebar does not exist in theme (using Inactive)', 'twizard-importer'
				);
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = esc_html__( 'Site does not support widget', 'twizard-importer' ); // explain why widget not imported
				}

				// Filter to modify settings object before conversion to array and import
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below)
				// Ideally the newer wie_widget_settings_array below will be used instead of this
				$widget = apply_filters( 'twizard_importer_widget_settings', $widget ); // object

				// Convert multidimensional objects to multidimensional arrays
				// Some plugins like Twizpack Widget Visibility store settings as multidimensional arrays
				...
				$widget = apply_filters( 'twizard_importer_widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = esc_html__( 'Widget already exists', 'twizard-importer' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					do_action( 'twizard_data_widget_after_widget_import', $after_widget_import );

					// Success message
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message = esc_html__( 'Imported', 'twizard-importer' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'twizard-importer' );
					}
				...
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'twizard-importer' ); // show "No Title" if widget instance is untitled
		...
		// Hook after import
		do_action( 'twizard_importer_after_process_widget', $sidebars_widgets );

		// Return results
		return apply_filters( 'twizard_data_widget_import_results', $results );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		$processed_posts     = twiz_cache()->get( 'posts', 'mapping' );
		$processed_user_slug = twiz_cache()->get( 'user_slug', 'mapping' );
		$processed_terms     = twiz_cache()->get( 'terms', 'mapping' );
		$remap_posts         = twiz_cache()->get( 'posts', 'requires_remapping' );
		 ...
		 * @param array
		 */
		$skip_post = apply_filters( 'twizard_import_skip_post', false, $data );
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'twizard-importer' ),
				$data['post_title'],
				$data['post_type']
			) );
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__('%s "%s" already exists.', 'twizard-importer'),
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%s" (%s)', 'twizard-importer' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		if ( $requires_remapping ) {
			...
			twiz_cache()->update( 'posts', $remap_posts, 'requires_remapping' );
		}
		$this->mark_post_exists( $data, $post_id );
		twiz_cache()->update( 'posts', $processed_posts, 'mapping' );

		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'twizard-importer' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'twizard-importer' ),
			$original_id,
			$post_id
		) );
	...
	protected function process_author( $data, $meta ) {
		...
		$processed_user_slug = twiz_cache()->get( 'user_slug', 'mapping' );
		$processed_users     = twiz_cache()->get( 'users', 'mapping' );
		...
		$login              = $original_slug;
		$user_slug_override = twiz_cache()->get( 'user_slug_override' );
	...
	protected function process_term( $data, $meta ) {
		...
		$processed_terms     = twiz_cache()->get( 'terms', 'mapping' );
		$processed_term_id   = twiz_cache()->get( 'term_id', 'mapping' );
		$processed_term_slug = twiz_cache()->get( 'term_slug', 'mapping' );
		$remap_terms         = twiz_cache()->get( 'terms', 'requires_remapping' );
		...
		twiz_cache()->update( 'terms', $processed_terms, 'mapping' );
		twiz_cache()->update( 'term_id', $processed_term_id, 'mapping' );
		twiz_cache()->update( 'term_slug', $processed_term_slug, 'mapping' );
		twiz_cache()->update( 'terms', $remap_terms, 'requires_remapping' );
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		$num_comments       = 0;
		$processed_comments = twiz_cache()->get( 'comments', 'mapping' );
		$processed_users    = twiz_cache()->get( 'users', 'mapping' );
		$remap_comments     = twiz_cache()->get( 'comments', 'requires_remapping' );
		...
		twiz_cache()->update( 'comments', $processed_comments, 'mapping' );
		twiz_cache()->update( 'comments', $remap_comments, 'requires_remapping' );
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		$remap_meta = twiz_cache()->get( 'posts_meta', 'requires_remapping' );
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( '_thumbnail_id' == $key ) {
					twiz_cache()->update( $post_id, (int) $value, 'featured_images' );
				}
			}
		}

		twiz_cache()->update( 'posts_meta', $remap_meta, 'requires_remapping' );
	...
	protected function maybe_add_remap_item( $meta_key, $requires_remapping, $object_id, $context ) {
		...
		$remap_meta = twiz()->get_setting( 'remap', $search_key );
	...
	protected function process_menu_item_meta( $post_id, $data, $meta ) {
		...
		$processed_term_id  = twiz_cache()->get( 'term_id', 'mapping' );
		$processed_posts    = twiz_cache()->get( 'posts', 'mapping' );
		$missing_items      = twiz_cache()->get( 'missing_menu_items' );
		$remap_posts        = twiz_cache()->get( 'posts', 'requires_remapping' );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'twizard-importer' ) );
		}
		...
		twiz_cache()->update( $remote_url, $upload['url'], 'url_remap' );

		// If we have a HTTPS URL, ensure the HTTP URL gets replaced too
		if ( substr( $remote_url, 0, 8 ) === 'https://') {
			$insecure_url = 'http' . substr( $remote_url, 5 );
			twiz_cache()->update( $insecure_url, $upload['url'], 'url_remap' );
		}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'twizard-importer' ),
					$code,
		...
		if ( 0 == $filesize ) {
			...
			return new WP_Error(
				'import_file_error',
				__( 'Zero size file downloaded', 'twizard-importer' )
			);
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf( __( 'Remote file is too large, limit is %s', 'twizard-importer' ), size_format( $max_size ) )
			);
		}
	...
	protected function post_exists( $data ) {
		...
		$existing_posts = twiz_cache()->get( 'posts', 'exists' );
		...
		twiz_cache()->update( 'posts', $existing_posts, 'exists' );
	...
	protected function comment_exists( $data ) {
		...
		$existing_comments = twiz_cache()->get( 'comments', 'exists' );
		...
		twiz_cache()->update( 'comments', $existing_comments, 'exists' );
	...
	protected function term_exists( $data ) {
		...
		$existing_terms = twiz_cache()->get( 'terms', 'exists' );
		...
		twiz_cache()->update( 'terms', $existing_terms, 'exists' );
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'twizard-importer' ) );
		}
