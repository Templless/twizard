...
if ( ! class_exists( 'Twizard_WXR_Exporter' ) ) {

	/**
	 * Define Twizard_WXR_Exporter class
	 */
	class Twizard_WXR_Exporter {
		...
		public function get_options_to_export() {
			...
			if ( null === $this->export_options ) {
				...
				$default_options = apply_filters( 'twizard_data_export_options', array(
					'blogname',
					'blogdescription',
					'users_can_register',
					'posts_per_page',
				...
				$user_options = twiz()->get_setting( array( 'export', 'options' ) );
		...
		public function do_export( $into_file = true ) {
			...
			$use_custom_export = apply_filters( 'twizard_data_use_custom_export', false );
		...
		public function get_filename() {
			...
			return apply_filters( 'twizard_data_export_filename', 'sample-' . $template . '-' . $date . '.xml' );
		...
		private function add_extra_data( $xml ) {
			 ...
			 */
			do_action( 'twizard-importer/export/custom-data' );
		...
		public function get_tables() {
			...
			$user_tables = twiz()->get_setting( array( 'export', 'tables' ) );
		...
		private function get_widgets() {
			...
			$data = apply_filters( 'twizard_data_export_pre_get_widgets', $sidebars_widget_instances );

			// Encode the data for file contents
			$encoded_data = json_encode( $data );
			$encoded_data = apply_filters( 'twizard_data_export_get_widgets', $encoded_data );
...
/**
 * Returns instance of Twizard_WXR_Exporter
 *
 * @return object
 */
function twiz_exporter() {
	return Twizard_WXR_Exporter::get_instance();
}
