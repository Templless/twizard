...
if ( ! class_exists( 'Twizard_Importer_Cache' ) ) {

	/**
	 * Define Twizard_Importer_Cache class
	 */
	class Twizard_Importer_Cache {
		 ...
		 * @var string
		 */
		public $base_group = 'twizard-importer';
		...
		public function __construct() {
			...
			$this->handlers = array(
				'session' => 'Twizard_Importer_Session_Cache',
				'file'    => 'Twizard_Importer_File_Cache',
			);

			require_once twiz()->path( 'includes/cache-handlers/class-twizard-importer-cache-handler.php' );
			...
			if ( isset( $this->handlers[ $method ] ) ) {
				...
				$handler = 'Twizard_Importer_Session_Cache';
			}

			$file = $this->get_file_name( $handler );

			require_once twiz()->path( 'includes/cache-handlers/' . $file );
...
/**
 * Returns instance of Twizard_Importer_Cache
 *
 * @return object
 */
function twiz_cache() {
	return Twizard_Importer_Cache::get_instance();
}
